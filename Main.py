    ...
    try:
        ...
        if not matches:
            ...
            for inn in scorecards:
                ...
                show_batsmen_key = f"show_batsmen_{selected_match_id}_{bat_team}"
                show_bowlers_key = f"show_bowlers_{selected_match_id}_{bat_team}"
                show_partnerships_key = f"show_partnerships_{selected_match_id}_{bat_team}"
                hide_batsmen_key = f"hide_batsmen_{selected_match_id}_{bat_team}"
                hide_bowlers_key = f"hide_bowlers_{selected_match_id}_{bat_team}"
                hide_partnerships_key = f"hide_partnerships_{selected_match_id}_{bat_team}"
                ...
                with button_col1:
                    if st.button("🏏 Show Batsmen", key=show_batsmen_key):
                        st.session_state[show_batsmen_key] = True
                
                with button_col2:
                    if st.button("🎯 Show Bowlers", key=show_bowlers_key):
                        st.session_state[show_bowlers_key] = True
                
                with button_col3:
                    if st.button("🤝 Show Partnerships", key=show_partnerships_key):
                        st.session_state[show_partnerships_key] = True

                if st.session_state.get(show_batsmen_key, False):
                    batsmen_data = []
                    ...
                    if st.button("Hide Batsmen", key=hide_batsmen_key):
                        st.session_state[show_batsmen_key] = False

                if st.session_state.get(show_bowlers_key, False):
                    bowlers_data = []
                    ...
                    if st.button("Hide Bowlers", key=hide_bowlers_key):
                        st.session_state[show_bowlers_key] = False

                if st.session_state.get(show_partnerships_key, False):
                    partnerships = []
                    ...
                    if st.button("Hide Partnerships", key=hide_partnerships_key):
                        st.session_state[show_partnerships_key] = False
